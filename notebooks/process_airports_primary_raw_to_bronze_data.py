...
# MAGIC
# MAGIC Version History of  process_airports_primary_raw_to_struct_data
# MAGIC    
# MAGIC    Changes:
# MAGIC
# MAGIC      Developer: Pradeep Phulari
# MAGIC      Date Created: 05/2/2024
# MAGIC      Purpose: Load history
...
dbutils.widgets.text("param_file_loc","wasbs://fltcorepkg@baneausstgfltcorepkg.blob.core.windows.net/dev/config/","Parameter_File_Location")
...
dbutils.widgets.text("From_FILE_CYCLE_RELEAS_NBR","2301","From_FILE_CYCLE_RELEAS_NBR")
dbutils.widgets.text("To_FILE_CYCLE_RELEAS_NBR","2405","To_FILE_CYCLE_RELEAS_NBR")
...
# dbutils.widgets.text("From_FILE_CYCLE_RELEAS_NBR","2303","From_FILE_CYCLE_RELEAS_NBR")
# dbutils.widgets.text("To_FILE_CYCLE_RELEAS_NBR","2303","To_FILE_CYCLE_RELEAS_NBR")
...
From_FILE_CYCLE_RELEAS_NBR = dbutils.widgets.get("From_FILE_CYCLE_RELEAS_NBR")
To_FILE_CYCLE_RELEAS_NBR = dbutils.widgets.get("To_FILE_CYCLE_RELEAS_NBR")
...
try :
    ...
    tmpRawNavigDF = rawNavigDF.selectExpr(          "substring(_c0,1,1) as RecordType",
													...
													"substring(_c0,7,4) as AirportICAOIdentifier",
													"substring(_c0,11,2) as ICAOCode",
													"substring(_c0,13,1) as SubsectionCode",
													"substring(_c0,14,3) as ATA_IATADesignator",
													"substring(_c0,17,2) as ReservedExpansion",
													"substring(_c0,19,3) as BlankSpacing2",
													"substring(_c0,22,1) as ContinuationRecordNumber",
													"substring(_c0,23,5) as SpeedLimitAltitude",
													"substring(_c0,28,3) as LongestRunway",
													"substring(_c0,31,1) as IFRCapability",
													"substring(_c0,32,1) as LongestRunwaySurfaceCode",
													"substring(_c0,33,9) as AirportReferencePtLatitude",
													"substring(_c0,42,10) as AirportReferencePtLongitude",
													"substring(_c0,52,5) as MagneticVariation",
													"substring(_c0,57,5) as AirportElevation",
													"substring(_c0,62,3) as SpeedLimit",
													"substring(_c0,65,4) as RecommendedNavaid",
													"substring(_c0,69,2) as ICAOCode2",
													"substring(_c0,71,5) as TransitionsAltitude",
													"substring(_c0,76,5) as TransitionLevel",
													"substring(_c0,81,1) as PublicMilitaryIndicator",
													"substring(_c0,82,3) as TimeZone",
													"substring(_c0,85,1) as DaylightIndicator",
													"substring(_c0,86,1) as MagneticTrueIndicator",
													"substring(_c0,87,3) as DatumCode",
													"substring(_c0,90,4) as ReservedExpansion2",
													"substring(_c0,94,30) as AirportName",
													"substring(_c0,124,5) as FileRecordNumber",
													"substring(_c0,129,4) as CycleDate" ).where( ( col("_c0").substr(22, 1).isin('0','1') )  &  ( col("_c0").substr(5,1) == "P" ) &  ( col("_c0").substr(13,1) == "A" ) )          
 
  # Create record create date and timestamp, inputfile name column for snapshot date
...
SAVE_PATH = SUCCESS_PATH
STATUS_CD = "S"
MSG_DESC = "All Airport Primary records has been successfully read"

log_operational_data(SYS_NM, NOTEBOOK_NM, CLUSTER_NM, CLUSTER_ID, SRC_FILE_NM, TARGET_NM, START_TMS, END_TMS, TARGET_TYPE_CD, STATUS_CD, MSG_DESC, TARGET_ADLS_ZONE, RUN_ID, NOTEBOOK_JOB_URL, SAVE_PATH)

# COMMAND ----------

# DBTITLE 1,Get the distinct snapshot date from incoming Airport Primary data
...
# DBTITLE 1,Save Airport Primary Records
# Save Airport Primary Records
...
try:
  ...
  SAVE_PATH = SUCCESS_PATH
  STATUS_CD = "S"
  MSG_DESC = "Airport Primary records has been successfully written in Struct zone"
  TARGET_NM = ""
...
# DBTITLE 1,Create the Airport Primary delta table (if not already exist) in Struct database
...
SAVE_PATH = SUCCESS_PATH
STATUS_CD = "S"
MSG_DESC = "Notebook completed processing all Airport Primary records"
END_TMS = str(datetime.now())
